...
class CalendarioTest {
    ...
    void testAggiungiAppuntamentoValido() {
        ...
        assertDoesNotThrow(
                () -> calendario.aggiungiAppuntamento(animale, "Vaccino", "Vaccinazione annuale", inizio, fine));
    }
    ...
    void testAggiungiAppuntamentoNonValido() {
        ...
        assertThrows(IllegalArgumentException.class,
                () -> calendario.aggiungiAppuntamento(animale, "Test", "Desc", baseDate, baseDate.minusHours(1)));

        // 2. Durata < 1 ora
        assertThrows(IllegalArgumentException.class,
                () -> calendario.aggiungiAppuntamento(animale, "Test", "Desc", baseDate, baseDate.plusMinutes(30)));

        // 3. Orario non intero (es. 9:15)
        assertThrows(IllegalArgumentException.class, () -> calendario.aggiungiAppuntamento(animale, "Test", "Desc",
        ...
        assertThrows(IllegalArgumentException.class,
                () -> calendario.aggiungiAppuntamento(animale, "Test", "Desc", earlyStart, earlyStart.plusHours(1)));

        // 5. Fuori orario lavorativo (dopo le 18)
        LocalDateTime lateEnd = LocalDateTime.of(LocalDate.now().plusDays(1), LocalTime.of(19, 0));
        assertThrows(IllegalArgumentException.class,
                () -> calendario.aggiungiAppuntamento(animale, "Test", "Desc", lateEnd.minusHours(1), lateEnd));
    ...
    void testSovrapposizioneParziale() {
        ...
        assertThrows(SovrapposizioneAppuntamentoException.class,
                () -> calendario.aggiungiAppuntamento(animale, "Secondo", "Desc", inizio, inizio.plusHours(1)));

        // 11:00 - 12:00 (Sovrapposto)
        assertThrows(SovrapposizioneAppuntamentoException.class,
                () -> calendario.aggiungiAppuntamento(animale, "Terzo", "Desc", inizio.plusHours(1), fine));
        ...
        assertDoesNotThrow(
                () -> calendario.aggiungiAppuntamento(animale, "Adiacente Dopo", "Desc", fine, fine.plusHours(1)));
    }
}
